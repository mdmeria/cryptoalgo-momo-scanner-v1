...
def trading_cycle(pos_mgr: PositionManager,
    ...
    for sym in sorted(all_symbols):
        ...
        for setup in found_setups:
            ...
            # Momentum: limit order entry with 2-candle confirmation + 10-min expiry
            if strat == "momentum":
                limit_key = f"{sym}_momentum"
                if limit_key not in pending_limits:
                    import time as _time
                    momo_depth_alt = None
                    if depth_data:
                        try:
                            momo_depth_alt = get_depth_alternative(
                                sym, depth_data, current_close,
                                setup["side"], strat)
                        except Exception:
                            pass
                    # Compute 0.75R cancel price
                    ...
                    pending_limits[limit_key] = {
                        **setup,
                        ...
                        "confirm_count": 0,
                        "confirmed": False,
                        "depth_alt": momo_depth_alt,
                    }
                    logger.info("  PENDING MOMO %s %s: entry=%.6g tp=%.6g(%.2f%%) "
                                "sl=%.6g(%.2f%%) RR=%.2f DPS=%d(%d%d%d) R2=%.3f ch2h=%.3f "
                                "ch15=%.4f (awaiting 2-bar confirm)",
                                setup["side"].upper(), sym,
                                setup["entry"], setup["tp"], setup["tp_pct"],
                                setup["sl"], setup["sl_pct"],
                                setup["rr"], setup["dps_total"],
                                setup.get("dps_dur", 0), setup.get("dps_app", 0),
                                setup.get("dps_vol", 0),
                                setup.get("r2_2h", 0), setup.get("channel_2h", 0),
                                setup.get("channel_15m", 0))
                    if momo_depth_alt:
                        logger.info("    DEPTH alt: tp=%.6g(%.2f%%) sl=%.6g(%.2f%%) RR=%.2f wall=$%s",
                                    momo_depth_alt["depth_tp"], momo_depth_alt["depth_tp_pct"],
                                    momo_depth_alt["depth_sl"], momo_depth_alt["depth_sl_pct"],
                                    momo_depth_alt["depth_rr"],
                                    f"{momo_depth_alt['depth_sl_wall_usd']:,.0f}")
                continue  # don't open position yet — wait for confirmation + fill
    ...
    # --- Manage pending limit orders (MR_CHOP + Momentum) ---
    import time as _time
    expired_keys = []
    for limit_key, lim in list(pending_limits.items()):
        ...
        side_lim = lim["side"]
        strat_lim = lim.get("strategy", "mr_chop")

        if sym_lim not in candle_cache or len(candle_cache[sym_lim]) == 0:
            continue
        ...
        # --- Momentum: 2-candle confirmation + limit fill + 0.75R cancel ---
        if strat_lim == "momentum":
            # Phase 1: 2-candle confirmation (each cycle = ~1 candle)
            if not lim.get("confirmed", False):
                ...
                if onside:
                    ...
                    # Confirmation broken — cancel
                    logger.info("  MISSED MOMO %s %s: confirm failed bar %d (close=%.6g vs entry=%.6g)",
                                side_lim.upper(), sym_lim, lim.get("confirm_count", 0) + 1,
                                last_close, limit_price)
                    expired_keys.append(limit_key)
                    continue

                if lim["confirm_count"] >= 2:
                    lim["confirmed"] = True
                    lim["confirmed_at"] = _time.time()
                    logger.info("  CONFIRMED MOMO %s %s: 2-bar confirm passed, limit=%.6g (10min expiry)",
                                side_lim.upper(), sym_lim, limit_price)
                continue  # still confirming, check next cycle

            # Phase 2: confirmed — check 0.75R cancel first
            cancel_price = lim.get("cancel_075r_price", 0)
            if side_lim == "long" and last_high >= cancel_price:
                logger.info("  MISSED MOMO %s %s: price=%.6g moved 0.75R onside (cancel=%.6g) after %.0fs",
                            side_lim.upper(), sym_lim, limit_price, cancel_price, elapsed)
                expired_keys.append(limit_key)
                continue
            if side_lim == "short" and last_low <= cancel_price:
                logger.info("  MISSED MOMO %s %s: price=%.6g moved 0.75R onside (cancel=%.6g) after %.0fs",
                            side_lim.upper(), sym_lim, limit_price, cancel_price, elapsed)
                expired_keys.append(limit_key)
                continue

            # Phase 3: check if limit fills (pullback reaches entry level)
            filled = False
            if side_lim == "long" and last_low <= limit_price:
                filled = True
            elif side_lim == "short" and last_high >= limit_price:
                filled = True
            ...
            if filled:
                ...
                logger.info("  LIMIT FILLED MOMO %s %s: fill=%.6g waited=%.0fs R2=%.3f ch=%.3f DPS=%d",
                            side_lim.upper(), sym_lim, limit_price, elapsed,
                            lim.get("r2_2h", 0), lim.get("channel_2h", 0),
                            lim.get("dps_total", 0))
                expired_keys.append(limit_key)
                continue

            # Phase 4: expiry (10 minutes from placement)
            if elapsed >= lim.get("limit_expiry_mins", 10) * 60:
                logger.info("  MISSED MOMO %s %s: price=%.6g expired after %.0fs",
                            side_lim.upper(), sym_lim, limit_price, elapsed)
                expired_keys.append(limit_key)
            continue

        # --- MR_CHOP: simple fill check ---
        if side_lim == "short" and last_high >= limit_price:
            pos_mgr.open_position(lim)
            log_trade_entry(lim)
            ...
            expired_keys.append(limit_key)
            continue
        elif side_lim == "long" and last_low <= limit_price:
            pos_mgr.open_position(lim)
            log_trade_entry(lim)
            logger.info("  LIMIT FILLED MR_CHOP %s %s: fill=%.6g waited=%.0fs",
                        side_lim.upper(), sym_lim, limit_price, elapsed)
        ...
        # MR_CHOP expired? (3 minutes default)
