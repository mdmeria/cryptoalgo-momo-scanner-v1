...
OUTPUT_CSV = "zct_momo_results_v12.csv"
LOG_FILE = "zct_momo_log_v12.txt"
...
MIN_2H_MOVE_PCT = 3.0  # price must have moved >=3% in last 2h
...
def simulate_momo_live_entry(df_c, df_h, df_l, signal_idx, side,
    ...
    # Phase 1: 1-bar confirmation (signal bar = first close, need 1 more)
    # Signal bar already closed beyond level. Next bar must also close beyond.
    confirm_bar = None
    for cb in range(signal_idx + 1, min(signal_idx + 15, n)):
        if side == "long" and df_c[cb] > level:
            ...
            # Next bar closed back through — level rejected
    ...
    fill_bar = None
    for fb in range(confirm_bar, min(confirm_bar + 20, n)):
...
def process_symbol(sym, dataset_dir, market_score_data=None):
    ...
    # Rolling 5-min volume (raw vol is already USD on Binance futures)
    vol_5m = np.zeros(n)
    for j in range(5, n):
        vol_5m[j] = vol[j - 5:j].sum()
    ...
    for i in range(scan_start, scan_end):
        ...
        for direction, level in signals:
            ...
            stair_sm30_w = smma30[max(0, i - 119):i + 1]
            if len(stair_sm5) >= 60:
                # SMMA5 on correct side of SMMA30 for 80%+
                if is_long:
                    sm5_pct = np.sum(stair_sm5 > stair_sm30_w) / len(stair_sm5) * 100
                else:
                    sm5_pct = np.sum(stair_sm5 < stair_sm30_w) / len(stair_sm5) * 100
                if sm5_pct < 80:
                    continue

                # SMMA5 must be trending (R² > 0.80)
                n_sm5 = len(stair_sm5)
            ...
            # ── GATE: Staircase step detection over last 2h ──
            # Split 2h into 15-min segments (8 windows).
            # Each window: check if it made a higher-low (long) or lower-high (short)
            # AND lower close (short) / higher close (long) vs previous window.
            if i < 120:
                continue
            n_segs = 8  # 8 x 15-bar segments = 120 bars (2h)
            seg_size = 15
            seg_lows = []
            seg_highs = []
            seg_closes = []
            ...
            # Count steps: higher-low + higher-close (long) or lower-high + lower-close (short)
            if is_long:
            ...
            # Need at least 5 of 7 steps holding (allow 2 breaks)
            if steps < 5:  # 5 of 7 steps must hold
                continue
            if not dir_ok:
                continue
            if not no_deep_pb:
            ...
            # ── GATE: Last 15 min grind — directional efficiency > 0.4 ──
            last15 = c[i - 14:i + 1]
            ...
            if eff_15 < 0.4:
                continue
            # Last 15 must be in correct direction
            if is_long and last15[-1] < last15[0]:
                continue
            if not is_long and last15[-1] > last15[0]:
                continue

            # ── (approach quality checks removed — SMMA5 R² handles quality) ──
